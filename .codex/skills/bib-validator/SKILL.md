---
name: bib-validator
description: "Validate and correct BibTeX reference entries against Google Scholar. Use this skill whenever a user wants to check, validate, verify, or clean up a .bib file or BibTeX references. Triggers include: check my references, validate my .bib file, verify my bibliography, fix my BibTeX entries, check my citations against Scholar, or any time a .bib file is uploaded or mentioned alongside words like validate, check, verify, correct, or update. Always use this skill when the user has a .bib file and wants to make sure the references are accurate and up to date."
---
...
Save the report as `bib_validation_report.md` in `/mnt/user-data/outputs/` (or the project's preferred output directory). Use `present_files` to share it with the user. Do not write to or modify the original .bib file.
