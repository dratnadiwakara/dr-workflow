...
description: >
  ...
  table values. Trigger this skill whenever the user wants to: check that numbers cited in the body of
  a paper match what's in the tables, verify significance/sign claims against table results, catch
  copy-paste or rounding errors before journal submission, or produce a structured audit report of
  all in-text quantitative claims. Also trigger when the user says things like "check my numbers",
  "verify my in-text citations match the tables", "consistency check", or "audit the paper against
  the tables". Input is a single .tex file, or a paper .tex file plus a separate tables .tex file
  (the latter is \input{}-ed into the former).
---
