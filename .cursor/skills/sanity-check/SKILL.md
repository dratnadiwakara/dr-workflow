...
description: >
  ...
  This skill must be explicitly invoked — it does NOT auto-trigger.
  Use when the user says "sanity check", "check my data", "validate my regression data",
  "check this dataset", or similar phrases requesting data quality verification.
  Covers two modes: (1) checking data behind a specific regression in an R script,
  or (2) checking the output dataset of a data-construction script.
  Produces an R script and a Markdown report in correspondence/sanity-check/.
---
...
Create an R script at `correspondence/sanity-check/sanity_check_[descriptor].R` where
...
Write the report to `correspondence/sanity-check/sanity_check_[descriptor].md`.
...
**Date:** [auto-generated]
**Script:** `correspondence/sanity-check/sanity_check_[descriptor].R`
