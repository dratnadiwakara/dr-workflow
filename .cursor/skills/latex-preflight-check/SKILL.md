...
- Path to the **main LaTeX document** to review (e.g. `docs/paper/main_20260312.tex`).
...
- Figures and tables referenced by the manuscript, such as:
  - PDFs/PNGs in `docs/figures/`, `figures/`, or adjacent directories.
  - `.tex` tables in `docs/tables/` or similar.
...
You may read from:
- `docs/` (figures/tables/results artifacts),
...
- You may create **exactly one** output file: the markdown report at
  `correspondence/latex-preflight/YYYY-MM-DD_filename.md`
- You may create the `correspondence/latex-preflight/` directory if it does not exist.
...
- Locate the corresponding output if possible (priority order):
  1) Exhibit file under `docs/figures/`, `docs/tables/`, `docs/results/` or similar
...
- `correspondence/latex-preflight/YYYY-MM-DD_filename.md`
- Use a short descriptive filename based on the LaTeX file (e.g., `main_20260312` → `main_20260312_preflight`).
