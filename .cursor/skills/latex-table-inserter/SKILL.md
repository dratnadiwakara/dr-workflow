...
1. **Target LaTeX file path** (required)  
   - Example: `latex/_tables_figures_Jan2026.tex`.
...
The inserted block **must** follow this structure, modeled on lines 353–376 of `_tables_figures_Jan2026.tex`:
...
- Keep `\vspace{0.25cm}` and `\resizebox{0.8\textwidth}{!}{ ... }` unless explicitly instructed otherwise, matching the canonical example.
...
3. **Preserve surrounding formatting.**  
   - Match indentation and spacing style used in the example block at lines 353–376.
...
## Example Target Style (from current project)

Use this existing table at lines 353–376 as the canonical style reference:
