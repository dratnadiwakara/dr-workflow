...
- Path to the **main LaTeX file** or a LaTeX snippet containing the result (e.g. `docs/paper/main_20260312.tex`).
...
- You must **not** read row-level datasets from any other directory (e.g. `data/processed/`, `docs/`, `temp/`, or absolute paths).
...
- `code/prof-check/` — your R scripts, logs, and non-data artifacts.
- `code/prof-check/tmp/` — optional temporary data artifacts **only if essential**.
- `correspondence/professor-robustness-check/` — markdown robustness reports.

Rules:
- You must **never modify or delete** user-authored files.
- Prefer in-memory processing.
- If you write temporary data files (only if essential), they must be created in `code/prof-check/tmp/` and:
...
In your R script(s) under `code/prof-check/`:
...
- Store scripts under `code/prof-check/`
...
```bash
'C:\Program Files\R\R-4.4.1\bin\Rscript.exe' 'code/prof-check/your_script_name.R'
````

Fallback (non-Windows / portable):

```bash
Rscript 'code/prof-check/your_script_name.R'
```
...
* write any logs/artifacts only under `code/prof-check/`.
...
* `correspondence/prof-check/YYYY-MM-DD_result-label.md`
...
- The RA must instead **create and run their own reconciliation scripts** under `code/prof-check/`, which may:
...
- `code/prof-check/<result-label>_reconcile.R` (and, if needed, small helper scripts),
