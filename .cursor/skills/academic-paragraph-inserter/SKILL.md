...
description: >
  ...
  at a specified line in a .tex file. Fits the insertion seamlessly — matching voice, tense,
  notation, and narrative flow — and communicates the author's point. Use when the user wants
  to add a sentence or paragraph at a given location, provides a .tex path and line number,
  or says "add at line N", "insert here", "add a sentence about X at line Y". Only the
  specified line and immediately surrounding text may be changed; do not edit anywhere else.
---
...
- **Path to the .tex file** — the manuscript to edit (e.g. `latex/paper_Jan2026.tex` or a body file such as `latex/_body_Jan2026.tex`).
...
- If the file is split across multiple `\input{}` or `\include{}` files, determine which file contains the line (e.g. the main .tex may `\input{_body_Jan2026.tex}`; line numbers refer to the file the user specified).
...
The examples below show how to draft the insertion text and connect it to context. In use, the user also provides the .tex path and line number; the insertion is then applied at that line only, with no other changes to the file.

### Example 1 — Single sentence, results section

**User's point:** "I want to add that the effect is stronger for smaller firms." (User also specifies e.g. `latex/_body_Jan2026.tex` and line 142.)

**Preceding text:** "...Column 3 adds firm fixed effects and our estimates are
largely unchanged, with the coefficient on leverage remaining at 0.038 (t = 4.2)."

**Following text:** "Table 5 presents the results for the subsample of investment-grade
firms..."

**Insertion:**
> Consistent with financial constraints amplifying the effect, we find that the
> relationship is approximately twice as large for firms in the bottom size quintile
> (Column 4), where the coefficient on leverage rises to 0.071 (t = 3.8).

**Placement:** After the sentence ending "...coefficient on leverage remaining at 0.038
(t = 4.2)."

**Rationale:** The insertion adds the heterogeneity finding immediately after the baseline
result it qualifies, and the mention of Column 4 connects to the table structure. The
transition into Table 5 (investment-grade firms) flows naturally because the size-quintile
analysis is a separate dimension.

---

### Example 2 — New paragraph, introduction

**User's point:** "I want to add that our paper also contributes to the literature on information asymmetry, which I forgot to mention." (User also specifies the .tex file and the line after the value-added paragraph, before the road map.)

**Preceding text (last paragraph of antecedents/value-added section):** "...Overall,
our paper most closely relates to Baker and Wurgler (2002) and Greenwood and Hanson
(2013), who study how firms time equity markets, but we differ in focusing on the
debt side and using high-frequency data to isolate the timing channel."

**Following text (next paragraph, road map):** "The remainder of the paper proceeds
as follows. Section 2 describes the data..."

**Insertion:**
> Our paper also contributes to the literature on information asymmetry in debt markets.
> Myers and Majluf (1984) and subsequent work (Bharath, Pasquariello, and Wu, 2009)
> show that information asymmetry raises the cost of external finance. We extend this
> work by documenting that the effect is state-dependent: asymmetry amplifies the
> leverage-spread relationship in periods of high market uncertainty, a prediction
> we test using the VIX as a conditioning variable.

**Placement:** As a new paragraph after the existing antecedents/value-added paragraph,
before the road map.

**Rationale:** The contribution to information asymmetry is a distinct value-added point
and warrants its own paragraph at the end of the value-added section. Placing it before
the road map follows the standard introduction structure where all contributions are
stated before the paper's organization is outlined.
